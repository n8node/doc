...
import { getKieTaskRecord, parseKieResultJson, firstMediaUrlFromKieTaskResult } from "@/lib/generation/kie-image-client";
...
/**
 * GET /api/v1/generate/video/status?taskId=...
 */
export async function GET(request: NextRequest) {
  ...
  if (task.status === "success") {
    const billedCredits =
      task.billedCredits ??
      (task.costCredits != null ? applyGenerationMargin(task.costCredits, await getGenerationMarginPercent()) : null);
    return NextResponse.json({
      taskId: task.id,
      status: task.status,
