...
export default function AdminGenerationPage() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/v1/admin/generation/stats?limit=100&media=image");
      const data = await res.json();
      if (res.ok) {
  ...
  const imagePricingRows = useMemo(
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          API-ключ Kie.ai, наценка на кредиты, задачи и модели для изображений, прайс по моделям (без Kling —{" "}
          <a href="/admin/generation-video" className="text-primary underline">
            Генерация видео
          </a>
          ).
        </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            Прайс изображений (кредиты по моделям)
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            Модели генерации изображений. Цены Kling для видео настраиваются в разделе «Генерация видео». Синхронизация с{" "}
            <a href="https://kie.ai/pricing" target="_blank" rel="noopener noreferrer" className="underline">
              kie.ai/pricing
            </a>{" "}
            — «Обновить прайс», затем «Сохранить вручную».
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          {pricingFetchedAt && (
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Только изображения. Кто, когда, модель, стоимость. Последние 100 успешных генераций.
          </p>
        </CardHeader>
        <CardContent>
          {statsLoading ? (
          ...
          {statsTotal > 0 && (
            <p className="text-xs text-muted-foreground mt-2">Всего успешных генераций изображений: {statsTotal}</p>
          )}
        </CardContent>
      </Card>
