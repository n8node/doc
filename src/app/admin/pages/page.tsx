...
export default function AdminPagesPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/v1/admin/pages/${id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Публичные страницы</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Создание страниц, доступных всем пользователям по адресу /pages
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border bg-surface p-8 text-center">
          ...
          <Link href="/admin/pages/new" className="mt-4 inline-block">
