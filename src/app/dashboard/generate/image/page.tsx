    ...
    <div className="max-w-3xl mx-auto py-6 space-y-6">
      ...
      {(status === "processing" || status === "success" || status === "failed") && (
        <Card>
          ...
          <CardContent className="space-y-4">
            {status === "processing" && (
              <p className="text-muted-foreground">Генерация занимает до минуты. Результат появится здесь и сохранится в «Мои файлы».</p>
            )}
            {status === "failed" && (
            ...
            {status === "success" && (resultUrl || fileId) && (
              <div className="space-y-3">
                {resultUrl && (
                  <div>
                    <img src={resultUrl} alt="Результат" className="max-w-full rounded-lg border object-contain max-h-96" />
                  </div>
                )}
