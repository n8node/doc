...
function parseCalKey(token: string): { keyToken: string; fullKey: string } | null {
...
/**
 * Пользователь для API интеграций календаря: сессия или Bearer cal_...
 * Требуется plan feature calendar_bridge.
 */
export async function getCalendarBridgeUserId(
  request: NextRequest
): Promise<string | null> {
  const auth = request.headers.get("authorization");
  if (auth?.startsWith("Bearer ")) {
    const token = auth.slice(7).trim();
    const parsed = parseCalKey(token);
    if (!parsed) return null;

    const row = await prisma.calendarBridgeApiKey.findUnique({
    ...
    const allowed = await hasFeature(row.userId, "calendar_bridge");
    if (!allowed) return null;

    await prisma.calendarBridgeApiKey.update({
      where: { id: row.id },
  ...
  const allowed = await hasFeature(session.user.id, "calendar_bridge");
  if (!allowed) return null;
...
export async function createCalendarBridgeApiKey(
  userId: string,
  name: string
  ...
  const row = await prisma.calendarBridgeApiKey.create({
    data: {
      userId,
